...
class DemoDataSeeder extends Seeder
{
    ...
    {
        if (app()->isProduction() && ! (bool) config('bizhr.allow_demo_data', false)) {
    ...
    /**
     * @param  array<string, int>  $branches
     * @param  array<string, array<string, int>>  $departments
     * @return array<string, array<string, int>>
     */
    ...
    private function seedLeave(int $companyId, array $employees, array $users): void
    {
        ...
        $annual = $this->upsertId('leave_types', ['company_id' => $companyId, 'code' => 'ANNUAL'], [
            'name' => 'ឈប់សម្រាកប្រចាំឆ្នាំ', 'days_per_year' => 18, 'is_statutory_annual_leave' => true, 'is_paid' => true,
        ...
        $sick = $this->upsertId('leave_types', ['company_id' => $companyId, 'code' => 'SICK'], [
            'name' => 'ឈប់សម្រាកព្យាបាលជំងឺ', 'days_per_year' => 7, 'is_statutory_annual_leave' => false, 'is_paid' => true,
    ...
    {
        if (! Schema::hasTable('payroll_periods') || ! Schema::hasTable('payroll_items')) {
            return;
        }
    ...
    private function seedPerformance(int $companyId, array $positions, array $employees, array $users): void
    {
        ...
        $template = $this->upsertId('kpi_templates', [
            'company_id' => $companyId, 'position_id' => $positions['PP-HQ']['SALES'], 'name' => 'KPI ផ្នែកលក់ប្រចាំខែ',
        ], [
    ...
    private function seedExpenses(int $companyId, array $employees, array $users): void
    {
        ...
        foreach ($items as [$key,$date,$category,$amount,$currency,$purpose,$status]) {
            $managerReviewed = $status !== 'pending_manager';
            $accountingReviewed = in_array($status, ['approved', 'paid'], true);
            $paid = $status === 'paid';
            $this->upsertId('expense_claims', [
                ...
                'manager_id' => $managerReviewed ? $users['manager'] : null,
                'manager_reviewed_at' => $managerReviewed ? now()->subDays(2) : null,
                'accountant_id' => $accountingReviewed ? $users['accountant'] : null,
                'accountant_reviewed_at' => $accountingReviewed ? now()->subDay() : null,
                'review_note' => in_array($status, ['approved', 'paid'], true) ? 'បានពិនិត្យឯកសារត្រឹមត្រូវ' : null,
                'paid_at' => $paid ? now() : null,
                'payment_reference' => $paid ? 'DEMO-PAY-'.strtoupper($key) : null,
            ]);
        }
    }
    ...
    private function seedAnnouncements(int $companyId, array $branches, array $departments, array $users): void
    {
        ...
        foreach ($items as $index => [$title,$content,$audience,$department,$pinned]) {
            ...
            $this->upsertId('announcements', ['company_id' => $companyId, 'title' => $title], [
                ...
                'department_id' => $audience === 'department' ? $departments['PP-HQ'][$department] : null,
    ...
    private function upsertId(string $table, array $unique, array $values): int
    {
        ...
        $columns = $this->columns[$table] ??= array_values(Schema::getColumnListing($table));
        ...
        if (in_array('public_id', $columns, true)) {
            if ($existing && filled($existing->public_id ?? null)) {
