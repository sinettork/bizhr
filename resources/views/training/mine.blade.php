<x-layouts::app title="My Training & Courses">
    <x-workspace-command-bar title="My training plan" icon="fa-book-open" context="My Workspace / Learning">
        <x-slot:actions>
            <a class="btn btn-action-link btn-sm" href="{{ route('dashboard') }}">
                <i class="fa-solid fa-arrow-left"></i><span>Workspace</span>
            </a>
        </x-slot:actions>
    ...
    <div class="reference-list border-0 bg-transparent shadow-none" data-list-container>
        <div class="row g-3">
                    ...
                    <article class="profile-card h-100 mb-0 d-flex flex-column">
                        <div class="profile-card-body flex-grow-1">
                            <div class="d-flex align-items-start justify-content-between gap-3 mb-3">
                                <span class="metric-icon {{ $isCompleted ? 'bg-success-subtle text-success' : 'bg-primary-subtle text-primary' }}"><i class="fa-solid {{ $isCompleted ? 'fa-award' : 'fa-book-open-reader' }}"></i></span>
                                <span class="status-text text-bg-{{ $isCompleted ? 'success' : 'primary' }}">{{ str($enrollment->status)->replace('_', ' ')->title() }}</span>
                            </div>
                            <h2 class="h6 fw-bold mb-2">{{ $enrollment->course?->title }}</h2>
                            @if($enrollment->course?->description)
                                <p class="small text-body-secondary">{{ \Illuminate\Support\Str::limit($enrollment->course->description, 110) }}</p>
                            @endif
                            <div class="d-flex justify-content-between small mb-2"><span class="text-body-secondary">Progress</span><strong>{{ $enrollment->progress }}%</strong></div>
                            <div class="progress mb-3" style="height:6px"><div class="progress-bar {{ $isCompleted ? 'bg-success' : '' }}" style="width:{{ max(0, min(100, $enrollment->progress)) }}%"></div></div>
                        ...
                        <div class="card-footer bg-white d-flex justify-content-end">
                            @if(! $isCompleted)
                                <button class="btn btn-primary btn-sm" type="button" data-bs-toggle="modal" data-bs-target="#updateCourse{{ $enrollment->id }}"><i class="fa-solid fa-arrow-trend-up me-1"></i>Update progress</button>
                            @else
                                <span class="small text-success fw-semibold"><i class="fa-solid fa-circle-check me-1"></i>Learning completed</span>
                            @endif
                        </div>
                    </article>
                </div>
            ...
            <div class="modal fade" id="updateCourse{{ $enrollment->id }}" tabindex="-1">
                <div class="modal-dialog">
                    <form class="modal-content" method="POST" action="{{ route('training.progress', $enrollment) }}">
                        @csrf
                        <div class="modal-header"><h2 class="modal-title fs-5">Update progress · {{ $enrollment->course?->title }}</h2><button class="btn-close" type="button" data-bs-dismiss="modal"></button></div>
                        <div class="modal-body">
                            <div class="mb-3"><label class="form-label">Progress <span class="text-danger">*</span></label><div class="input-group"><input class="form-control" type="number" min="0" max="100" name="progress" value="{{ $enrollment->progress }}" required><span class="input-group-text">%</span></div></div>
                            <div><label class="form-label">Assessment score</label><input class="form-control" type="number" step=".01" min="0" max="100" name="score" value="{{ $enrollment->score }}" placeholder="Optional"></div>
                        </div>
