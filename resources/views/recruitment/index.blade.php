<x-layouts::app title="Recruitment">
    <x-workspace-command-bar title="Recruitment & Talent Acquisition" icon="fa-user-plus" context="Hiring Pipeline">
        <x-slot:filters>
            <form class="reference-filter-form" method="GET">
                <div class="input-group reference-search">
                    ...
                    <input class="form-control" name="search" value="{{ request('search') }}" placeholder="Search vacancies">
                </div>
        ...
        <x-slot:actions>
            @can('recruitment.manage')
                <x-list-actions :add-modal="'addVacancy'" add-label="Open vacancy" />
            @endcan
        </x-slot:actions>
    ...
    <section class="mb-4">
        <div class="d-flex align-items-end justify-content-between gap-3 mb-2">
            <div>
                <div class="small text-uppercase text-body-secondary fw-semibold">Open roles</div>
                <h2 class="h6 fw-bold mb-0">Vacancy overview</h2>
            </div>
            <span class="small text-body-secondary">{{ $vacancies->total() }} vacancy(s)</span>
        </div>
                    ...
                    <article class="profile-card h-100 mb-0">
                        <div class="profile-card-body">
                            <div class="d-flex justify-content-between gap-3 mb-2">
                                <div>
                                    <h3 class="h6 fw-bold mb-1">{{ $vacancy->title }}</h3>
                                    <div class="small text-body-secondary">
                                        {{ $vacancy->position?->title ?: 'General role' }}
                                        @if($vacancy->branch)
                                            <span class="mx-1">·</span>{{ $vacancy->branch->name }}
                                        @endif
                                    </div>
                                </div>
                                <span class="status-text text-bg-{{ $vacancy->status === 'open' ? 'success' : 'secondary' }}">
                                    {{ str($vacancy->status)->title() }}
                                </span>
                            </div>

                            <div class="d-flex gap-4 small mt-3">
                                <div><span class="d-block text-body-secondary">Openings</span><strong>{{ $vacancy->openings }}</strong></div>
                                <div><span class="d-block text-body-secondary">Candidates</span><strong>{{ $vacancy->applicants_count }}</strong></div>
                                <div><span class="d-block text-body-secondary">Closes</span><strong>{{ $vacancy->close_date?->format('d M') ?: 'Open' }}</strong></div>
                            </div>
                        </div>

                        @can('recruitment.manage')
                            <div class="card-footer bg-white border-top d-flex justify-content-end">
                                <button class="btn btn-action-link btn-sm" type="button" data-bs-toggle="modal" data-bs-target="#addApplicant{{ $vacancy->id }}">
            ...
            @empty
                <div class="col-12">
                    <div class="profile-card mb-0">
                        <div class="profile-card-body text-center text-body-secondary py-5">
                            <i class="fa-solid fa-briefcase fa-xl d-block mb-3 text-primary"></i>No vacancies match this view.
                        </div>
                    </div>
                </div>
            @endforelse
    ...
    <section>
        <div class="d-flex align-items-end justify-content-between gap-3 mb-2">
            <div>
                <div class="small text-uppercase text-body-secondary fw-semibold">Candidate journey</div>
                <h2 class="h6 fw-bold mb-0">Hiring pipeline</h2>
            </div>
            <span class="small text-body-secondary">{{ $applicants->count() }} active/recent candidate(s)</span>
        </div>
                    ...
                    <div class="profile-card h-100 mb-0">
                        <div class="profile-card-header">
                            <h3 class="profile-card-title">
                                <i class="fa-solid {{ $icon }} text-primary"></i><span>{{ $label }}</span>
                            </h3>
                        ...
                        <div class="profile-card-body bg-body-tertiary p-2 d-grid gap-2 align-content-start" style="min-height: 420px;">
                            @forelse($rows as $applicant)
                                <article class="card border-0 shadow-sm">
                                    <div class="card-body p-3">
                                        ...
                                        <div class="small text-body-secondary mb-2">{{ $applicant->vacancy->title }}</div>
                                        <div class="small text-body-secondary mb-2">
                                            <i class="fa-regular fa-envelope me-1"></i>{{ $applicant->email }}
                                        </div>

                                        @if($applicant->hr_note)
                                            <p class="small text-body-secondary mb-2">
                                                <i class="fa-regular fa-note-sticky me-1"></i>{{ \Illuminate\Support\Str::limit($applicant->hr_note, 70) }}
                                            </p>
                                        @endif
                                        ...
                                        <div class="d-flex align-items-center justify-content-between gap-2 mt-3">
                                            ...
                                            <div class="dropdown">
                                                <button class="btn btn-action-link btn-sm" type="button" data-bs-toggle="dropdown" aria-expanded="false" aria-label="Candidate actions">
                                                    <i class="fa-solid fa-ellipsis"></i>
                                                </button>
                                                <ul class="dropdown-menu dropdown-menu-end shadow-sm">
                                                    @if($applicant->cv_path)
                                                        <li>
                                                            <a class="dropdown-item" href="{{ route('recruitment.applicants.cv', $applicant) }}" target="_blank">
                                                                <i class="fa-solid fa-file-arrow-down me-2"></i>Open CV
                                                            </a>
                                                        </li>
                                                    @endif
                                                        ...
                                                        @foreach($nextStages[$applicant->status] ?? [] as $stage)
                                                            <li>
                                                                <form method="POST" action="{{ route('recruitment.applicants.transition', $applicant) }}">
                                                                    ...
                                                                    <button class="dropdown-item {{ in_array($stage, ['rejected', 'declined'], true) ? 'text-danger' : '' }}" type="submit">
                            ...
                            @empty
                                <div class="text-center text-body-secondary small py-5">No candidates.</div>
                            @endforelse
                        </div>
                    </div>
                </div>
        ...
        @if(($groupedApplicants->get(null, collect()))->isNotEmpty())
            <div class="small text-body-secondary mt-2">Rejected/declined candidates are retained in history but omitted from the active pipeline.</div>
        @endif
    </section>
