...
new #[Title('ប្រភេទការឈប់សម្រាក')] class extends Component
{
    ...
    public function updated(
        ...
        if (
            $property === 'carry_forward_allowed'
    ...
    public function edit(int $leaveTypeId): void
    {
        ...
        $this->days_per_year = (string) $leaveType->days_per_year;
        $this->is_paid = (bool) $leaveType->is_paid;
        $this->requires_attachment = (bool) $leaveType->requires_attachment;
        $this->carry_forward_allowed = (bool) $leaveType->carry_forward_allowed;
        $this->maximum_carry_forward_days = (string) $leaveType->maximum_carry_forward_days;
        $this->is_active = (bool) $leaveType->is_active;
    ...
    public function save(): void
    {
        ...
        $validated['days_per_year'] = (float) $validated['days_per_year'];
        $validated['is_paid'] = (bool) $validated['is_paid'];
        $validated['requires_attachment'] = (bool) $validated['requires_attachment'];
        $validated['carry_forward_allowed'] = (bool) $validated['carry_forward_allowed'];
        $validated['maximum_carry_forward_days'] = $validated['carry_forward_allowed']
            ? (float) $validated['maximum_carry_forward_days']
            : 0;
...
<div class="w-full space-y-6 p-4 sm:p-6">
    ...
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        >
            <p class="text-sm text-zinc-500">ប្រភេទសរុប</p>
            <p
        ...
        <div
            class="rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-700 dark:bg-zinc-900"
        >
            <p class="text-sm text-zinc-500">កំពុងប្រើប្រាស់</p>
        ...
        <div
            class="rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-700 dark:bg-zinc-900"
        >
            <p class="text-sm text-zinc-500">មានប្រាក់ឈ្នួល</p>
        ...
        <div
            class="rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-700 dark:bg-zinc-900"
        >
            <p class="text-sm text-zinc-500">អាចយោងទៅឆ្នាំក្រោយ</p>
            <p class="mt-2 text-3xl font-semibold text-amber-600">
                {{ number_format($this->statistics['carry_forward']) }}
            </p>
        </div>
    </div>
    ...
    @if ($showForm)
        ...
        >
            ...
            <div
                class="mt-5 grid gap-4 rounded-xl bg-zinc-50 p-4 sm:grid-cols-2 xl:grid-cols-4 dark:bg-zinc-800"
            >
                <flux:checkbox
                    wire:model="is_paid"
            ...
            <table class="w-full text-left text-sm">
                <thead class="bg-zinc-50 dark:bg-zinc-800">
                    <tr>
                        ...
                        <th class="px-5 py-4 font-medium">លក្ខខណ្ឌ</th>
                        <th class="px-5 py-4 font-medium">ការប្រើប្រាស់</th>
                        <th class="px-5 py-4 font-medium">ស្ថានភាព</th>
                        <th class="px-5 py-4 text-right font-medium">សកម្មភាព</th>
                    </tr>
                </thead>
                ...
                >
                    @forelse ($this->leaveTypes as $leaveType)
                        ...
                        >
                            ...
                            <td class="px-5 py-4">
                                <div>
                                    {{ number_format((float) $leaveType->days_per_year, 1) }} ថ្ងៃ/ឆ្នាំ
                                </div>

                                @if ($leaveType->carry_forward_allowed)
                                    <div class="mt-1 text-xs text-zinc-500">
                                        យោងបានអតិបរមា
                                        {{ number_format((float) $leaveType->maximum_carry_forward_days, 1) }} ថ្ងៃ
                                    </div>
                                @endif
                            </td>
                            ...
                            <td class="px-5 py-4">
                                <div class="flex flex-wrap gap-1.5">
                                    <flux:badge
                                        size="sm"
                                        :color="$leaveType->is_paid ? 'green' : 'zinc'"
                                    >
                                        {{ $leaveType->is_paid
                                            ? 'មានប្រាក់ឈ្នួល'
                                            : 'គ្មានប្រាក់ឈ្នួល' }}
                            ...
                            <td class="px-5 py-4">
                                <div>
                                    {{ number_format($leaveType->requests_count) }} សំណើ
                                </div>

                                <div class="mt-1 text-xs text-zinc-500">
                                    {{ number_format($leaveType->balances_count) }} សមតុល្យ
                                </div>
                            </td>
                            ...
                            <td class="px-5 py-4">
                                <flux:badge
                                    size="sm"
                                    :color="$leaveType->is_active ? 'green' : 'zinc'"
                                >
                                    {{ $leaveType->is_active
                                        ? 'កំពុងដំណើរការ'
                                        : 'បានបិទ' }}
                        ...
                        <tr>
                            <td colspan="6" class="px-5 py-14 text-center">
                                ...
                                <p class="mt-2 text-sm text-zinc-500">
                                    បន្ថែមប្រភេទច្បាប់ថ្មី ឬប្តូរលក្ខខណ្ឌស្វែងរក។
                                </p>
                            </td>
                        </tr>
                    @endforelse
