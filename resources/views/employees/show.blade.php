    ...
    <ul class="nav profile-nav-tabs" role="tablist">
        ...
        <li class="nav-item" role="presentation">
            <button class="profile-nav-tab" id="tab-documents-btn" data-bs-toggle="tab" data-bs-target="#tab-documents" type="button" role="tab" aria-controls="tab-documents" aria-selected="false">
                ...
                <span class="badge text-bg-light border ms-1">{{ $employee->documents->count() }}</span>
            </button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="profile-nav-tab" id="tab-history-btn" data-bs-toggle="tab" data-bs-target="#tab-history" type="button" role="tab" aria-controls="tab-history" aria-selected="false">
                ...
                <span class="badge text-bg-light border ms-1">{{ $employee->employmentHistories->count() }}</span>
            </button>
        </li>
    </ul>
    ...
    <div class="tab-content">
                                ...
                                <div class="profile-hero-meta">
                                    <div class="d-flex align-items-center gap-2 flex-wrap mb-1">
                                        ...
                                        <span class="badge text-bg-{{ $employee->is_active ? 'success' : 'secondary' }}">
                                            {{ str($employee->employment_status ?: ($employee->is_active ? 'Active' : 'Inactive'))->title() }}
                                        </span>
                                    </div>
                                    <div class="profile-hero-sub fw-semibold">
                                        <code>{{ $employee->employee_code }}</code>
                                        @if($employee->position)
                                            <span class="mx-1">·</span><span>{{ $employee->position->title }}</span>
                                        @endif
                                        @if($employee->department)
                                            <span class="mx-1">·</span><span>{{ $employee->department->name }}</span>
                                        @endif
                                    </div>
                                    <div class="profile-hero-contact">
                                        @if($employee->gender)
                                            <span><i class="fa-solid fa-venus-mars me-1 text-body-secondary"></i>{{ ucfirst($employee->gender) }}</span>
                                        @endif
                                        @if($employee->email)
                                            <a href="mailto:{{ $employee->email }}"><i class="fa-regular fa-envelope me-1 text-body-secondary"></i>{{ $employee->email }}</a>
                                        @endif
                                        @if($employee->phone)
                                            <a href="tel:{{ $employee->phone }}"><i class="fa-solid fa-phone me-1 text-body-secondary"></i>{{ $employee->phone }}</a>
                                        @endif
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row g-2">
                                <div class="col-sm-6">
                                    <div class="profile-kv-row">
                                        ...
                                        <span class="profile-kv-value">{{ $employee->date_of_birth?->format('d M Y') ?? '—' }}</span>
                                    </div>
                                    <div class="profile-kv-row">
                                        <span class="profile-kv-label">Age</span>
                                        <span class="profile-kv-value">{{ $employee->date_of_birth ? $employee->date_of_birth->age.' years' : '—' }}</span>
                                    </div>
                                    <div class="profile-kv-row">
                                        <span class="profile-kv-label">Gender</span>
                                        <span class="profile-kv-value">{{ $employee->gender ? ucfirst($employee->gender) : '—' }}</span>
                                    </div>
                                </div>
                                <div class="col-sm-6">
                                    <div class="profile-kv-row">
                                        <span class="profile-kv-label">National ID</span>
                                        <span class="profile-kv-value font-monospace">{{ $employee->national_id ?: '—' }}</span>
                                    </div>
                                    <div class="profile-kv-row">
                                        <span class="profile-kv-label">Passport</span>
                                        <span class="profile-kv-value font-monospace">{{ $employee->passport_number ?: '—' }}</span>
                                    </div>
                                    <div class="profile-kv-row">
                                        <span class="profile-kv-label">City</span>
                                        <span class="profile-kv-value">{{ $employee->city ?: '—' }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="profile-card h-100 mb-0">
                        <div class="profile-card-header">
                            ...
                            @if(auth()->user()->can('employee.edit'))
                                <a href="{{ route('employees.edit', $employee) }}" class="btn btn-action-link btn-sm">
                                    <i class="fa-solid fa-pen"></i><span>Edit</span>
                                </a>
                            @endif
                        </div>
                        <div class="profile-card-body">
                            <div class="profile-kv-row">
                                ...
                                <span class="profile-kv-value text-break">{{ $employee->address ?: 'Not specified' }}</span>
                            </div>
                            <div class="profile-kv-row">
                    ...
                    <div class="profile-card h-100 mb-0">
                        ...
                        <div class="profile-card-body">
                            <div class="profile-kv-row">
                                ...
                                <span class="profile-kv-value">{{ $employee->emergency_contact_name ?: 'Not recorded' }}</span>
                            </div>
                            <div class="profile-kv-row">
                                <span class="profile-kv-label">Phone number</span>
                                <span class="profile-kv-value">
                                    @if($employee->emergency_contact_phone)
                                        <a href="tel:{{ $employee->emergency_contact_phone }}" class="text-decoration-none fw-semibold">
                            ...
                            <div class="profile-kv-row">
                                ...
                                <span class="profile-kv-value">{{ $employee->phone ?: '—' }}</span>
                            </div>
                            <div class="profile-kv-row">
                                <span class="profile-kv-label">Primary email</span>
                                <span class="profile-kv-value">{{ $employee->email ?: '—' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="profile-card h-100 mb-0">
                        ...
                        <div class="profile-card-body">
                            <div class="profile-kv-row">
                                ...
                                <span class="profile-kv-value">{{ $employee->branch?->name ?? '—' }}</span>
                            </div>
                            <div class="profile-kv-row">
                                <span class="profile-kv-label">Department</span>
                                <span class="profile-kv-value">{{ $employee->department?->name ?? '—' }}</span>
                            </div>
                            <div class="profile-kv-row">
                                <span class="profile-kv-label">Job title / Position</span>
                                <span class="profile-kv-value">{{ $employee->position?->title ?? '—' }}</span>
                            </div>
                            <div class="profile-kv-row">
                                <span class="profile-kv-label">Employment type</span>
                                <span class="profile-kv-value">{{ $employee->employmentType?->name ?? '—' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="profile-card h-100 mb-0">
                        ...
                        <div class="profile-card-body">
                            <div class="profile-kv-row">
                                ...
                                <span class="profile-kv-value">{{ $employee->hire_date?->format('d M Y') ?? '—' }}</span>
                            </div>
                            <div class="profile-kv-row">
                                <span class="profile-kv-label">Probation end date</span>
                                <span class="profile-kv-value">{{ $employee->probation_end_date?->format('d M Y') ?? '—' }}</span>
                            </div>
                            <div class="profile-kv-row">
                                <span class="profile-kv-label">Contract start</span>
                                <span class="profile-kv-value">{{ $employee->contract_start_date?->format('d M Y') ?? '—' }}</span>
                            </div>
                            <div class="profile-kv-row">
                                <span class="profile-kv-label">Contract end</span>
                                <span class="profile-kv-value">{{ $employee->contract_end_date?->format('d M Y') ?? 'Permanent / Ongoing' }}</span>
                            </div>
                            <div class="profile-kv-row">
                                <span class="profile-kv-label">Tenure</span>
                                <span class="profile-kv-value">{{ $employee->hire_date ? $employee->hire_date->diffForHumans(null, true) : '—' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="profile-card-body">
                            <div class="profile-kv-row">
                                ...
                                <span class="profile-kv-value fs-6 text-primary fw-bold">
                                    {{ $employee->base_salary ? number_format((float) $employee->base_salary, 2).' '.$employee->salary_currency : '—' }}
                                </span>
                            </div>
                            <div class="profile-kv-row">
                                <span class="profile-kv-label">Salary currency</span>
                                <span class="profile-kv-value font-monospace">{{ $employee->salary_currency ?: 'USD' }}</span>
                            </div>
                            <div class="profile-kv-row">
                                <span class="profile-kv-label">Payment method</span>
                                <span class="profile-kv-value">{{ str($employee->payment_method ?: 'Bank Transfer')->replace('_', ' ')->title() }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="profile-card h-100 mb-0">
                        ...
                        <div class="profile-card-body">
                            <div class="profile-kv-row">
                                ...
                                <span class="profile-kv-value">{{ $employee->bank_name ?: '—' }}</span>
                            </div>
                            <div class="profile-kv-row">
                                <span class="profile-kv-label">Account holder name</span>
                                <span class="profile-kv-value">{{ $employee->bank_account_name ?: '—' }}</span>
                            </div>
                            <div class="profile-kv-row">
                                <span class="profile-kv-label">Account number</span>
                                <span class="profile-kv-value font-monospace fw-bold">{{ $employee->bank_account_number ?: '—' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="tab-pane fade" id="tab-documents" role="tabpanel" aria-labelledby="tab-documents-btn">
            <div class="d-flex align-items-center justify-content-between mb-2">
                ...
                <a class="btn btn-action-link btn-sm" href="{{ route('employees.documents.index', $employee) }}">
                            ...
                            @forelse($employee->documents as $doc)
                                <tr>
                                    <td class="ps-3 fw-semibold text-dark">
                                        <i class="fa-regular fa-file-pdf text-danger me-2"></i>{{ $doc->title ?: $doc->original_name }}
                                    </td>
                                    <td><span class="badge text-bg-light border">{{ str($doc->document_type ?? 'General')->headline() }}</span></td>
                                    <td class="small text-body-secondary font-monospace">{{ $doc->original_name }}</td>
                                    <td class="small">{{ number_format($doc->file_size / 1024, 1) }} KB</td>
                                    <td class="small">{{ $doc->created_at->format('d M Y') }}</td>
                                    <td class="text-end pe-3">
                                        <a class="btn btn-action-link btn-sm" href="{{ route('employees.documents.download', [$employee, $doc]) }}" target="_blank">
        ...
        <div class="tab-pane fade" id="tab-history" role="tabpanel" aria-labelledby="tab-history-btn">
            <div class="d-flex align-items-center justify-content-between mb-2">
                ...
                <a class="btn btn-action-link btn-sm" href="{{ route('employees.history.index', $employee) }}">
                        ...
                        <tbody>
                            @forelse($employee->employmentHistories as $history)
                                <tr>
                                    ...
                                    <td><span class="badge text-bg-primary">{{ str($history->event_type)->replace('_', ' ')->title() }}</span></td>
                                    <td>
                                        <div>{{ $history->department?->name ?? '—' }}</div>
                                        <small class="text-body-secondary">{{ $history->position?->title ?? '—' }}</small>
                                    </td>
