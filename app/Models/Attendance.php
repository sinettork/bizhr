...
class Attendance extends Model
{
    ...
    public function calculateScheduledWorkMinutes(): int
    {
        ...
        $duration = (int) round($startDate->diffInMinutes($endDate));
    ...
    public function calculateLateness(): int
    {
        ...
        if ($scheduled) {
            $scheduledTime = Carbon::parse($this->work_date->format('Y-m-d').' '.$scheduled);
            $checkInTime = Carbon::parse($this->check_in_at);

            if ($checkInTime->greaterThan($scheduledTime)) {
                $lateMinutes = (int) round($checkInTime->diffInMinutes($scheduledTime, true)) - $grace;
    ...
    public function calculateEarlyLeave(): int
    {
        ...
        if ($scheduled) {
            ...
            if ($checkOutTime->lessThan($scheduledTime)) {
                $early = (int) round($scheduledTime->diffInMinutes($checkOutTime, true)) - $grace;
    ...
    public function calculateWorkedMinutes(): int
    {
        ...
        $minutes = (int) round(Carbon::parse($this->check_in_at)->diffInMinutes(Carbon::parse($this->check_out_at)));
