...
class AttendanceReportController extends Controller
{
    ...
    {
        abort_unless($request->user()?->can('attendance.report'), 403);
        ...
        $applyFilters = function (Builder $query) use (
            $companyId,
            $search,
            $branchId,
            $employeeId,
            ...
            if (filled($search)) {
                $query->whereHas('employee', function (Builder $employeeQuery) use ($companyId, $search): void {
                    $employeeQuery
        ...
        $attendanceQuery = Attendance::query()->with([
            'employee.branch',
            'employee.department',
            'employee.position',
            'branch',
        ...
        $present = (clone $statisticsQuery)->whereIn('status', ['present', 'late'])->count();
        $late = (clone $statisticsQuery)->where('status', 'late')->count();
        $absent = (clone $statisticsQuery)->where('status', 'absent')->count();
        $workedMinutes = (int) (clone $statisticsQuery)->sum('worked_minutes');
        $overtimeMinutes = (int) (clone $statisticsQuery)->sum('overtime_minutes');
        $lateMinutes = (int) (clone $statisticsQuery)->sum('late_minutes');
        $earlyLeaveMinutes = (int) (clone $statisticsQuery)->sum('early_leave_minutes');
