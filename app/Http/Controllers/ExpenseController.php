...
class ExpenseController extends Controller
{
    ...
    public function mine(Request $request): View
    {
        ...
        $claims = ExpenseClaim::query()->where('employee_id', $employee->id)->latest('expense_date')->paginate($this->perPage($request, 20))->withQueryString();
    ...
    public function store(Request $request, UploadedFileSecurityService $fileSecurity): RedirectResponse
    {
        $employee = $request->user()->employee;
        abort_unless($employee !== null, 403);
        $data = $request->validate(['expense_date' => ['required', 'date', 'before_or_equal:today'], 'category' => ['required', 'string', 'max:100'], 'amount' => ['required', 'numeric', 'gt:0'], 'currency' => ['required', 'in:USD,KHR'], 'business_purpose' => ['required', 'string', 'min:10', 'max:2000'], 'receipt' => ['required', 'file', 'mimes:pdf,jpg,jpeg,png', 'max:5120']]);
        $file = $request->file('receipt');
        abort_unless($file !== null, 422);
        $fileSecurity->assertSafe($file, 'receipt');
        $path = $file->store("private/expenses/{$employee->id}", 'local');
        ExpenseClaim::query()->create([...array_diff_key($data, ['receipt' => true]), 'company_id' => $employee->company_id, 'employee_id' => $employee->id, 'receipt_path' => $path, 'receipt_original_name' => $file->getClientOriginalName(), 'status' => 'pending_manager']);
    ...
    {
        abort_unless($claim->company_id === (int) Company::query()->value('id'), 404);
    ...
    public function receipt(Request $request, ExpenseClaim $claim): StreamedResponse
    {
        ...
        abort_unless(Storage::disk('local')->exists((string) $claim->receipt_path), 404);

        return Storage::disk('local')->download((string) $claim->receipt_path, $claim->receipt_original_name ?: 'receipt');
    }
}
