...
class AttendanceCorrectionController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $attendances = Attendance::query()->where('employee_id', $employee->id)->where(fn (Builder $query) => $query->whereNotNull('check_in_at')->orWhereNotNull('check_out_at'))->latest('work_date')->limit(90)->get();
        $requests = AttendanceCorrection::query()->with(['attendance', 'reviewedBy'])->where('employee_id', $employee->id)->latest()->paginate($this->perPage($request, 20))->withQueryString();
    ...
    public function store(Request $request): RedirectResponse
    {
        $employee = $request->user()->employee;
        abort_unless($employee !== null, 403);
        $data = $request->validate(['attendance_id' => ['required', 'integer'], 'requested_check_in' => ['nullable', 'date'], 'requested_check_out' => ['nullable', 'date'], 'reason' => ['required', 'string', 'min:5', 'max:2000']]);
        $attendance = Attendance::query()->whereKey($data['attendance_id'])->where('employee_id', $employee->id)->firstOrFail();
        ...
        AttendanceCorrection::query()->create(['attendance_id' => $attendance->id, 'employee_id' => $employee->id, 'requested_check_in' => $changedIn ? $checkIn : null, 'requested_check_out' => $changedOut ? $checkOut : null, 'reason' => trim($data['reason']), 'status' => 'pending']);
    ...
    public function review(Request $request): View
    {
        ...
        $query = AttendanceCorrection::query()->with(['attendance', 'employee.branch', 'employee.department']);
        if ($request->user()->hasRole('Manager') && ! $request->user()->hasAnyRole(['Owner', 'Super Admin', 'HR Administrator'])) {
            abort_unless($actor !== null, 403);
            $query->where('status', 'pending')->whereHas('employee', fn (Builder $employees) => $employees->where('department_id', $actor->department_id)->where('id', '!=', $actor->id));
        } else {
            $query->where('status', 'pending')->when($actor, fn (Builder $corrections) => $corrections->whereHas('employee', fn (Builder $employees) => $employees->where('company_id', $actor->company_id)));
        }

        return view('attendance.corrections.review', ['corrections' => $query->latest()->paginate($this->perPage($request, 20))->withQueryString()]);
    }
    ...
    {
        $employee = $request->user()->employee;
        abort_unless($employee !== null && $correction->employee_id === $employee->id, 404);
    ...
    private function authorizeCorrection(Request $request, AttendanceCorrection $correction): void
    {
        ...
        if ($request->user()->hasAnyRole(['Owner', 'Super Admin'])) {
            return;
        }
        $actor = $request->user()->employee;
        abort_unless($actor && $correction->employee()->where('company_id', $actor->company_id)->exists(), 404);
        if ($request->user()->hasRole('Manager') && ! $request->user()->hasRole('HR Administrator')) {
