...
class AttendanceQrController extends Controller
{
    public function display(Request $request): View
    {
        ...
        return view('attendance.qr.display', compact('branches'));
    }
    ...
    public function create(Request $request): RedirectResponse
    {
        ...
        $token = Str::random(80);
        $session = AttendanceQrSession::query()->create(['branch_id' => $branch->id, 'token_hash' => hash('sha256', $token), 'expires_at' => now()->addSeconds($data['lifetime_seconds'] ?? config('attendance.qr.session_lifetime_seconds', 45)), 'created_by' => $request->user()->id]);
