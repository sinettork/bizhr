...
class PerformanceReviewService
{
    ...
    public function approve(PerformanceReview $review, User $actor): PerformanceReview
    {
        ...
        if ($review->reviewer_id === $actor->id && ! $actor->hasRole('Super Admin')) {
            throw new DomainException('The reviewer cannot provide HR approval for the same review.');
        }
    ...
    public function acknowledge(PerformanceReview $review, User $actor, ?string $comment): PerformanceReview
    {
        ...
        if ($review->status !== 'hr_approved') {
            throw new DomainException('Only an HR-approved review can be acknowledged.');
        }
    ...
    public function reopen(PerformanceReview $review, User $actor, string $reason): PerformanceReview
    {
        ...
        if (mb_strlen(trim($reason)) < 15) {
            throw new DomainException('A detailed reopening reason of at least 15 characters is required.');
        }
    ...
    /** @param list<array{weight: int|float, score: int|float}> $criteria */
    public function calculateWeightedScore(array $criteria): float
    {
        ...
        if (abs($weight - 100) > 0.001) {
            throw new DomainException('Review weights must total exactly 100%.');
        }
        $total = 0.0;
        foreach ($criteria as $criterion) {
            $score = (float) $criterion['score'];
            if ($score < 1 || $score > 5) {
                throw new DomainException('Scores must be between 1 and 5.');
            }
