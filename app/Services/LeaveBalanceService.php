...
class LeaveBalanceService
{
    ...
    public function initializeForEmployeeAndType(
        ...
        if (! $balance->wasRecentlyCreated && $leaveType->is_statutory_annual_leave) {
            $balance->earned_days = $earnedDays;
            $balance->save();
        }
    ...
    /**
     * Calculate entitlement as at today for Cambodia's statutory annual leave.
     * Other leave types remain company-policy based and receive their configured
     * annual entitlement when their balance is initialized.
     */
    private function earnedDaysFor(
        Employee $employee,
        LeaveType $leaveType,
        int $year,
    ): float {
        $annualEntitlement = round((float) $leaveType->days_per_year, 2);

        if (! $leaveType->is_statutory_annual_leave || ! $employee->hire_date) {
        ...
        $asOf = $today->lessThan($yearEnd) ? $today : $yearEnd;
        $employmentStart = CarbonImmutable::parse($employee->hire_date)->startOfDay();
        $accrualStart = $employmentStart->greaterThan($yearStart) ? $employmentStart : $yearStart;

        if ($accrualStart->greaterThan($asOf)) {
            return 0;
        }

        $months = $accrualStart->startOfMonth()->diffInMonths($asOf->startOfMonth()) + 1;

        return round(min($annualEntitlement, ($annualEntitlement / 12) * $months), 2);
    }
