...
class DatabaseBackupCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        if ($statement === false) {
            throw new RuntimeException('Unable to run the backup integrity check.');
        }
        ...
        foreach ($tables as $table) {
            $statement = $pdo->query("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='{$table}'");
            if ($statement === false) {
                throw new RuntimeException("Unable to verify table '{$table}' in the backup.");
            }
